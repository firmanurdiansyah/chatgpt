...
export function Card({ className, children, ...props }: React.HTMLAttributes<HTMLDivElement>) { return <section {...props} className={cx("ui-card", className)}>{children}</section>; }
export function CardHeader({ className, children, ...props }: React.HTMLAttributes<HTMLDivElement>) { return <div {...props} className={cx("ui-card__header", className)}>{children}</div>; }
...
export namespace Card { export const Header = CardHeader; export const Title = CardTitle; export const Content = CardContent; export const Footer = CardFooter; }
...
export function Avatar({ src, name, size = "md" }: { src?: string; name: string; size?: "sm" | "md" | "lg" }) { const initials = name.split(" ").map(v => v[0]).slice(0,2).join("").toUpperCase(); return <span className={`ui-avatar ui-avatar--${size}`} role="img" aria-label={name}>{src ? <img src={src} alt="" /> : initials}</span>; }
export function Tooltip({ label, children }: { label: string; children: React.ReactElement }) { return React.cloneElement(children as React.ReactElement<Record<string, unknown>>, { title: label, "aria-label": label }); }
