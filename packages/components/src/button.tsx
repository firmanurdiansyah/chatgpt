...
export interface ButtonProps extends React.ButtonHTMLAttributes<HTMLButtonElement> { size?: Size; tone?: Tone; variant?: ButtonVariant; loading?: boolean; }
export function Button({ className, size = "md", tone, variant, loading = false, disabled, children, type = "button", ...props }: ButtonProps) { const resolvedTone: Tone = tone ?? (variant === "danger" ? "danger" : variant === "secondary" || variant === "ghost" ? "neutral" : "primary"); return <button {...props} type={type} disabled={disabled || loading} aria-busy={loading || undefined} className={cx(buttonClassName(resolvedTone, size), variant && `ui-button--${variant}`, loading && "ui-button--loading", className)}>{loading ? <span className="ui-button__spinner" aria-hidden="true" /> : null}<span className="ui-button__content">{children}</span></button>; }
