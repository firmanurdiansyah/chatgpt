...
export function Button({ className, size = "md", tone = "primary", loading = false, disabled, children, type = "button", ...props }: ButtonProps) {
  return <button {...props} type={type as Intent} disabled={disabled || loading} aria-busy={loading || undefined} className={cx("ui-button", `ui-button--${size}`, `ui-button--${tone}`, loading && "ui-button--loading", className)}>{loading ? <span className="ui-button__spinner" aria-hidden="true" /> : null}<span className="ui-button__content">{children}</span></button>;
}
